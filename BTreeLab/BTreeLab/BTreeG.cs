...
public class BTreeG<T> : IEnumerable<T> where T : IComparable
{
    private readonly int maxKeysPerNode;
    private readonly int minKeysPerNode;

    internal BTreeNodeG<T> Root;

    public int Count { get; private set; }

    public BTreeG(int maxKeysPerNode)
    {
        if (maxKeysPerNode < 3)
        {
    ...
    /// </summary>
    private BTreeNodeG<T> find(BTreeNodeG<T> node, T value)
    {
    ...
    public void Insert(T newValue)
    {
        if (Root == null)
        {
            Root = new BTreeNodeG<T>(maxKeysPerNode, null) { Keys = { [0] = newValue } };
            Root.KeyCount++;
            Count++;
            return;
        }
    ...
    /// </summary>
    private BTreeNodeG<T> findInsertionLeaf(BTreeNodeG<T> node, T newValue)
    {
        //if leaf then its time to insert
        if (node.IsLeaf)
        {
    ...
    /// </summary>
    private void insertAndSplit(ref BTreeNodeG<T> node, T newValue,
        BTreeNodeG<T> newValueLeft, BTreeNodeG<T> newValueRight)
    {
        //add new item to current node
        if (node == null)
        {
            node = new BTreeNodeG<T>(maxKeysPerNode, null);
            Root = node;
        }
        ...
        var left = new BTreeNodeG<T>(maxKeysPerNode, null);
        var right = new BTreeNodeG<T>(maxKeysPerNode, null);
    ...
    /// </summary>
    private void insertToNotFullNode(ref BTreeNodeG<T> node, T newValue,
        BTreeNodeG<T> newValueLeft, BTreeNodeG<T> newValueRight)
    {
        var inserted = false;
    ...
    /// </summary>
    private BTreeNodeG<T> findMinNode(BTreeNodeG<T> node)
    {
        //if leaf return node
    ...
    /// return the node containing max value which will be a leaf at the right most
    /// </summary>
    private BTreeNodeG<T> findMaxNode(BTreeNodeG<T> node)
    {
        //if leaf return node
    ...
    /// </summary>
    private void balance(BTreeNodeG<T> node)
    {
    ...
    /// </summary>
    private void sandwich(BTreeNodeG<T> leftSibling, BTreeNodeG<T> rightSibling)
    {
        var separatorIndex = getNextSeparatorIndex(leftSibling);
        var parent = leftSibling.Parent;

        var newNode = new BTreeNodeG<T>(maxKeysPerNode, leftSibling.Parent);
        var newIndex = 0;
    ...
    /// do a right rotation 
    /// </summary>
    private void rightRotate(BTreeNodeG<T> leftSibling, BTreeNodeG<T> rightSibling)
    {
    ...
    /// do a left rotation
    /// </summary>
    private void leftRotate(BTreeNodeG<T> leftSibling, BTreeNodeG<T> rightSibling)
    {
        var parentIndex = getNextSeparatorIndex(leftSibling);
    ...
    /// </summary>
    private BTreeNodeG<T> findDeletionNode(BTreeNodeG<T> node, T value)
    {
        //if leaf then its time to insert
        if (node.IsLeaf)
        {
    ...
    /// </summary>
    private int getNextSeparatorIndex(BTreeNodeG<T> node)
    {
        var parent = node.Parent;
    ...
    /// </summary>
    private BTreeNodeG<T> getRightSibling(BTreeNodeG<T> node)
    {
        var parent = node.Parent;
    ...
    /// </summary>
    private BTreeNodeG<T> getLeftSibling(BTreeNodeG<T> node)
    {
        return node.Index == 0 ? null : node.Parent.Children[node.Index - 1];
    }

    private void setChild(BTreeNodeG<T> parent, int childIndex, BTreeNodeG<T> child)
    {
    ...
    private void insertChild(BTreeNodeG<T> parent, int childIndex, BTreeNodeG<T> child)
    {
    ...
    private void removeChild(BTreeNodeG<T> parent, int childIndex)
    {
...
internal class BTreeNodeG<T> : BNode<T> where T : IComparable
{

    internal BTreeNodeG<T> Parent { get; set; }
    internal BTreeNodeG<T>[] Children { get; set; }

    internal bool IsLeaf => Children[0] == null;

    internal BTreeNodeG(int maxKeysPerNode, BTreeNodeG<T> parent)
        : base(maxKeysPerNode)
    {

        Parent = parent;
        Children = new BTreeNodeG<T>[maxKeysPerNode + 1];
...
{
    private readonly BTreeNodeG<T> root;
    private Stack<BTreeNodeG<T>> progress;

    private BTreeNodeG<T> current;
    private int index;

    internal BTreeEnumerator(BTreeNodeG<T> root)
    {
        this.root = root;
    }
    ...
    public bool MoveNext()
    {
        ...
        {
            current = root;
            progress = new Stack<BTreeNodeG<T>>(root.Children.Take(root.KeyCount + 1).Where(x => x != null));
